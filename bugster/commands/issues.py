...
def issues_command(
    ...
    try:
        ...
        if project_id is None:
            ...
            if not project_id:
                console.print(
                    "[red]No project ID found. Please run 'bugster init' first or provide a project ID with --project-id[/red]"
                )
                raise typer.Exit(1)
        ...
        with BugsterHTTPClient() as client:
            if history:
                ...
                if not response or not response.get("issues"):
                    console.print(
                        f"[yellow]No historical issues found for project {project_id}[/yellow]"
                    )
                    return
                ...
                if total_issues > 10:
                    ...
                    console.print(
                        f"[green]Found {total_issues} issues. All saved to {issues_file}[/green]"
                    )
                    return
                ...
                try:
                    response = client.get(
                        "/api/v1/issues", params={"project_id": project_id}
                    )
                except Exception as e:
                    if "404" in str(e):
                        console.print(
                            f"[yellow]No recent issues for project_id {project_id}[/yellow]"
                        )
                        return
                    raise
