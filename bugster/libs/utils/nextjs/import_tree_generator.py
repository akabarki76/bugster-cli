...
class ImportTreeGenerator:
    ...
    def save_to_json(self, tree: Dict, filename: str = "import_tree.json"):
        ...
        with open(summary_filename, "w", encoding="utf-8") as f:
            f.write("IMPORT TREE SUMMARY\n")
            f.write("=" * 50 + "\n\n")

            for entry_point, data in tree.items():
                f.write(f"Entry Point: {entry_point}\n")
                f.write(f"Local Imports: {data.get('import_count', 0)}\n")
                f.write("-" * 30 + "\n")
                self._write_tree_summary(data.get("imports", {}), f, indent=1)
                f.write("\n")

        print(f"Import tree summary saved to {summary_filename}")

    def _write_tree_summary(
        self, imports: Dict, file, indent: int = 0, visited: Set[str] = None
    ):
        """Write a summary of the tree to file."""
        if visited is None:
            visited = set()
        ...
        for import_path, import_data in imports.items():
            ...
            if isinstance(import_data, dict) and "path" in import_data:
                ...
                circular = import_data.get("circular", False)
                unresolved = import_data.get("unresolved", False)
                import_count = import_data.get("import_count", 0)
                ...
                if circular:
                    ...
                    status = f" ({import_count} local imports)"

                file.write(f"{prefix}{import_path} -> {path}{status}\n")

                if (
                    not circular
                    and not unresolved
                    and path not in visited
                    and "imports" in import_data
                ):
                    visited.add(path)
                    nested_imports = import_data["imports"]
                    if nested_imports:  # Only recurse if there are imports
                        self._write_tree_summary(
                            nested_imports, file, indent + 1, visited
                        )
