...
def install_missing_packages(include_packages, available_packages, debug=False):
    ...
    missing_packages = [pkg for pkg in include_packages if pkg not in get_package_names(available_packages)]
...
def parse_args():
    parser = argparse.ArgumentParser(description="Update setup.py with dependencies from requirements.txt")
    parser.add_argument("--req-file", default="requirements.txt", help="Path to requirements file")
    parser.add_argument("--exclude", nargs="+", help="List of packages to exclude")
    parser.add_argument("--include", nargs="+", help="List of packages to include (only these will be added)")
    parser.add_argument("--dev", action="store_true", help="Include dev dependencies (defaults to False)")
    parser.add_argument("--debug", action="store_true", help="Enable debug output")
    parser.add_argument("--no-install", action="store_true", help="Don't install missing packages")
...
if __name__ == "__main__":
    ...
    dev_dependencies = [
        ...
        "pylint", "autopep8", "pycodestyle", "pyflakes", "rope",
        "setuptools", "wheel", "pip", "build", "twine", "pyinstaller"
    ]
    ...
    if args.include and not args.no_install:
        all_requirements = install_missing_packages(args.include, all_requirements, args.debug)
